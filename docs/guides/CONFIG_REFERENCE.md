...
- `OPENAI_API_KEY`
- `OPENAI_BASE_URL`
- `OPENAI_TRANSCRIBE_API_KEY`（可选：仅用于语音转写；未设置则回退到 `OPENAI_API_KEY`）
- `OPENAI_TRANSCRIBE_BASE_URL`（可选：仅用于语音转写；未设置则回退到 `OPENAI_BASE_URL`）
- `VOICE_TRANSCRIBE_PROVIDER=auto|openai|sherpa`
- `VOICE_TRANSCRIBE_FORCE_ENABLED=0|1`
- `SHERPA_ASR_ENABLED=0|1`
- `SHERPA_ASR_MODE=off|local|remote`
- `SHERPA_ASR_REMOTE_URL`（仅 `remote` 模式）
- `SHERPA_ONNX_TOKENS`（仅 `local` 模式）
- `SHERPA_ONNX_WENET_CTC_MODEL`（仅 `local` 模式；WeNet CTC）
- `SHERPA_ONNX_WHISPER_ENCODER`（仅 `local` 模式；Whisper）
- `SHERPA_ONNX_WHISPER_DECODER`（仅 `local` 模式；Whisper）
- `SHERPA_ONNX_WHISPER_LANGUAGE`（可选）
- `SHERPA_ONNX_WHISPER_TASK`（默认 `transcribe`）
- `SHERPA_ONNX_WHISPER_TAIL_PADDINGS`（默认 `-1`）
- `SHERPA_ONNX_NUM_THREADS`（默认 `2`）
- `SHERPA_ONNX_DECODING_METHOD`（默认 `greedy_search`）
- `SHERPA_ONNX_DEBUG=0|1`
- `SHERPA_ONNX_SAMPLE_RATE`（默认 `16000`）
- `SHERPA_ONNX_FEATURE_DIM`（默认 `80`）
- `AI_MODEL`
- `AI_FALLBACK_MODELS`
...
当启用 `VOICE_TRANSCRIBE_FORCE_ENABLED=1` 时，后端会从 SystemConfig 读取并强制覆盖语音相关配置（见 `backend/app/services/voice_config_service.py`）：
