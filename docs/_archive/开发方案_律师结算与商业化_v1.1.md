...
- `docs/_archive/律师结算功能需求说明书.md`（结算/提现 MVP）
- `docs/_archive/百姓法律助手商业化策略与定价方案.md`（定价/会员/分成策略）
- `docs/_archive/百姓法律助手验收测试用例.md`（验收用例清单）
- `docs/_archive/补充协议模板.md`（范围确认与里程碑对齐）

当前交付版（v1.0）已具备：律师服务 MVP 闭环（预约-支付-确认-接单-完成-评价）、余额支付闭环、支付宝支付链接生成（需甲方参数）、支付回调审计/对账工具、合规页面与同意留痕、以及自动化回归门禁（pytest 95 passed / Playwright 76 passed / pyright+bpyright 0）。
...
- 验收用例与补充协议对齐（对应本方案 M3）：
  ...
  - 已将新增结算/商业化用例补入 `docs/_archive/百姓法律助手验收测试用例.md`
  - 已更新 `docs/_archive/补充协议模板.md` 与交付口径对齐，并补充验收/交付模板
...
- 数据模型草案（ER 说明）
- API 清单（含权限）
- 验收用例映射表（对应 `docs/_archive/百姓法律助手验收测试用例.md`）
...
- 将新增结算/商业化用例补入 `docs/_archive/百姓法律助手验收测试用例.md`
- 在 `docs/_archive/补充协议模板.md` 中，将里程碑 M1/M2 的交付内容与当前实现对齐
- 新增交付辅助文档：
