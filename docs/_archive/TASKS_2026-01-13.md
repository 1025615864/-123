...
> 归档：本文件已于 **2026-01-13** 结项归档（快照：`docs/_archive/TASKS_2026-01-13.md`）。当前进行中的任务请查看：`TASKS_NEXT.md`。
...
- [x] 建议：补一个“未登录态全站路由 smoke”Playwright 用例（不依赖登录），纳入 CI（用于提前发现 Console 红字/静态资源 404）
  - 已做：新增 Playwright 用例：`frontend/tests/e2e/unauth-route-smoke.spec.ts`
  - 已做：覆盖 publicRoutes + restrictedRoutes；对 restrictedRoutes 校验重定向到 `/login?return_to=...`
  - 已做：CI 已纳入：`.github/workflows/ci.yml` 执行 `npm run test:e2e -- --grep "(documents:|unauth:)"`
- [x] 建议：对“需要登录”但可被直接访问的页面，统一做 page-level guard（避免进入页面后再发起 401 请求）
  - 已做：`frontend/src/App.tsx` 对受限路由统一套 `RequireAuth`（避免页面先渲染再打 401）
  - 已做：`frontend/src/components/AdminLayout.tsx` 自带管理员登录/角色 guard（未登录重定向；非管理员给出页面态）
