...
class TestUserAPI:
    ...
    async def test_login_rate_limited(self, client: AsyncClient):
        from app.utils.rate_limiter import RateLimitConfig, rate_limiter
        ...
        max_requests, _window_seconds = RateLimitConfig.AUTH_LOGIN

        last_status = None
        for _ in range(int(max_requests) + 1):
            res = await client.post("/api/user/login", json=login_data)
            last_status = res.status_code

        assert last_status == 429

    @pytest.mark.asyncio
    async def test_admin_user_toggle_active_writes_audit_log(
        self,
        client: AsyncClient,
        ...
        target = User(
            ...
            nickname="audit_target",
            hashed_password=hash_password("Test123456"),
            role="user",
            is_active=True,
        )
        test_session.add_all([admin, target])
        ...
        res = await client.put(
            f"/api/user/admin/{int(target.id)}/toggle-active",
        ...
        q = await test_session.execute(
            select(AdminLog)
            .where(AdminLog.user_id == int(admin.id))
            .order_by(AdminLog.id.desc())
            .limit(5)
        )
        logs = list(q.scalars().all())
        assert any(int(getattr(l, "target_id", 0) or 0) == int(target.id) for l in logs)
...
class TestSystemMetrics:
    @pytest.mark.asyncio
    async def test_system_metrics_requires_admin(self, client: AsyncClient, test_session: AsyncSession):
        from app.models.user import User
        from app.utils.security import create_access_token, hash_password
        ...
        anon_res = await client.get("/api/system/metrics")
        ...
        text = str(admin_res.text)
        assert "baixing_ai_errors_total" in text
        assert "baixing_ai_chat_requests_total" in text
...
class TestCalendarAPI:
    ...
    @pytest.mark.asyncio
    async def test_calendar_reminders_validation(
        self,
        client: AsyncClient,
        test_session: AsyncSession,
    ):
        ...
        u1 = User(
            username="calendar_v_u1",
            email="calendar_v_u1@example.com",
            nickname="calendar_v_u1",
            hashed_password=hash_password("Test123456"),
            role="user",
            is_active=True,
        )
        ...
        due_at = datetime.now(timezone.utc) + timedelta(days=1)
        remind_after_due = due_at + timedelta(hours=1)
        ...
        bad_create = await client.post(
            ...
            json={
                ...
                "note": None,
                "due_at": due_at.isoformat(),
                "remind_at": remind_after_due.isoformat(),
            },
            headers={"Authorization": f"Bearer {token_u1}"},
        )
        ...
        create_ok = await client.post(
            ...
            json={
                ...
                "note": None,
                "due_at": due_at.isoformat(),
                "remind_at": (due_at - timedelta(minutes=10)).isoformat(),
            },
            headers={"Authorization": f"Bearer {token_u1}"},
        )
        assert create_ok.status_code == 200
        rid = _as_int(_json_dict(create_ok).get("id"))

        bad_update = await client.put(
            f"/api/calendar/reminders/{rid}",
            json={"remind_at": remind_after_due.isoformat()},
            headers={"Authorization": f"Bearer {token_u1}"},
        )
        assert bad_update.status_code == 400

        from_at = due_at + timedelta(days=2)
        to_at = due_at + timedelta(days=1)
        bad_list = await client.get(
            "/api/calendar/reminders",
            params={"from_at": from_at.isoformat(), "to_at": to_at.isoformat()},
            headers={"Authorization": f"Bearer {token_u1}"},
        )
        assert bad_list.status_code == 400


class TestDocumentHistoryAPI:
    @pytest.mark.asyncio
    async def test_documents_save_list_and_permissions(
        self,
        client: AsyncClient,
        test_session: AsyncSession,
    ):
        ...
        anon_res = await client.post(
            "/api/documents/save",
            json={"document_type": "complaint", "title": "t", "content": "c"},
        )
        ...
        save_res = await client.post(
            ...
            json={
                ...
                "title": "民事起诉状",
                "content": "正文",
                "payload": {"case_type": "劳动纠纷"},
            },
            headers={"Authorization": f"Bearer {token_u1}"},
        )
        assert save_res.status_code == 200
        saved = _json_dict(save_res)
        doc_id = _as_int(saved.get("id"))
        assert doc_id > 0
        ...
        detail_res = await client.get(
            f"/api/documents/my/{doc_id}",
            headers={"Authorization": f"Bearer {token_u1}"},
        )
        assert detail_res.status_code == 200
        detail = _json_dict(detail_res)
        assert _as_int(detail.get("id")) == doc_id
        assert _as_int(detail.get("user_id")) == u1.id
...
class TestAIShareAPI:
    ...
    async def test_share_link_create_and_public_read(
        ...
        res_revoke_anon = await client.post(f"/api/ai/consultations/{sid}/share/revoke")
        assert res_revoke_anon.status_code == 401

        res_revoke_forbidden = await client.post(
            f"/api/ai/consultations/{sid}/share/revoke",
            headers={"Authorization": f"Bearer {token_u2}"},
        )
        assert res_revoke_forbidden.status_code == 403

        res_revoke_ok = await client.post(
            f"/api/ai/consultations/{sid}/share/revoke",
            headers={"Authorization": f"Bearer {token_u1}"},
        )
        assert res_revoke_ok.status_code == 200
        revoke_payload = _json_dict(res_revoke_ok)
        assert revoke_payload.get("revoked") is True

        res_shared_revoked = await client.get(f"/api/ai/share/{token}")
        assert res_shared_revoked.status_code == 401
        ...
        assert res_ok_2.status_code == 200
        payload_2 = _json_dict(res_ok_2)
        token_2 = str(payload_2.get("token") or "")
        assert token_2.strip() != ""
        assert token_2 != token

        res_shared_2 = await client.get(f"/api/ai/share/{token_2}")
        assert res_shared_2.status_code == 200
    ...
    @pytest.mark.asyncio
    async def test_ai_consultations_favorite_toggle_and_filter(
        self,
        client: AsyncClient,
        test_session: AsyncSession,
    ):
        ...
        token = create_access_token({"sub": str(u1.id)})

        c1 = Consultation(user_id=u1.id, session_id="s_fav_1", title="A")
        c2 = Consultation(user_id=u1.id, session_id="s_fav_2", title="B")
        ...
        items0 = cast(object, res0.json())
        assert isinstance(items0, list)
        assert len(cast(list[dict[str, object]], items0)) >= 2

        fav1 = await client.post(
            "/api/ai/consultations/s_fav_1/favorite",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert fav1.status_code == 200
        assert cast(dict, fav1.json()).get("is_favorite") is True

        res_fav_only = await client.get(
            "/api/ai/consultations",
            params={"favorites_only": True},
            headers={"Authorization": f"Bearer {token}"},
        )
        assert res_fav_only.status_code == 200
        items_fav_only = cast(object, res_fav_only.json())
        assert isinstance(items_fav_only, list)
        sids = {str(x.get("session_id")) for x in cast(list[dict[str, object]], items_fav_only)}
        assert sids == {"s_fav_1"}
        assert all(bool(x.get("is_favorite")) for x in cast(list[dict[str, object]], items_fav_only))

        fav2 = await client.post(
            "/api/ai/consultations/s_fav_1/favorite",
            headers={"Authorization": f"Bearer {token}"},
        )
        assert fav2.status_code == 200
        assert cast(dict, fav2.json()).get("is_favorite") is False

        res_fav_only2 = await client.get(
            "/api/ai/consultations",
            params={"favorites_only": True},
            headers={"Authorization": f"Bearer {token}"},
        )
        assert res_fav_only2.status_code == 200
        items_fav_only2 = cast(object, res_fav_only2.json())
        assert isinstance(items_fav_only2, list)
        assert len(items_fav_only2) == 0
    ...
    @pytest.mark.asyncio
    async def test_ai_transcribe_missing_api_key_returns_503(
        self,
        client: AsyncClient,
        test_session: AsyncSession,
        monkeypatch: MonkeyPatch,
        ...
        user = User(
            username="transcribe_no_key_u1",
            email="transcribe_no_key_u1@example.com",
            nickname="u1",
            hashed_password=hash_password("Test123456"),
            role="user",
            is_active=True,
        ...
        monkeypatch.setattr(ai_router.settings, "openai_api_key", "", raising=False)
        ...
        payload = _json_dict(res)
        assert payload.get("error_code") == "AI_NOT_CONFIGURED"
        assert isinstance(res.headers.get("X-Request-Id"), str)

    @pytest.mark.asyncio
    async def test_ai_transcribe_empty_audio_returns_400(
        self,
        client: AsyncClient,
        test_session: AsyncSession,
        monkeypatch: MonkeyPatch,
        ...
        user = User(
            username="transcribe_empty_u1",
            email="transcribe_empty_u1@example.com",
            nickname="u1",
            hashed_password=hash_password("Test123456"),
            role="user",
            is_active=True,
        ...
        monkeypatch.setattr(ai_router.settings, "openai_api_key", "test_key", raising=False)
        ...
        assert res.status_code == 400
        payload = _json_dict(res)
        assert payload.get("error_code") == "AI_BAD_REQUEST"
        assert isinstance(res.headers.get("X-Request-Id"), str)

    @pytest.mark.asyncio
    async def test_ai_transcribe_file_read_failure_returns_400(
        self,
        client: AsyncClient,
        test_session: AsyncSession,
        monkeypatch: MonkeyPatch,
        ...
        user = User(
            username="transcribe_bad_read_u1",
            email="transcribe_bad_read_u1@example.com",
            nickname="u1",
            hashed_password=hash_password("Test123456"),
            role="user",
            is_active=True,
        ...
        async def bad_read(self: object, *args: object, **kwargs: object) -> bytes:
            _ = self
            _ = args
            _ = kwargs
            raise RuntimeError("boom")

        monkeypatch.setattr(StarletteUploadFile, "read", bad_read, raising=True)
    ...
    @pytest.mark.asyncio
    async def test_ai_chat_persist_failure_still_returns_200(
        self,
        client: AsyncClient,
        test_session: AsyncSession,
        monkeypatch: MonkeyPatch,
        ...
        user = User(
            username="chat_persist_fail_u1",
            email="chat_persist_fail_u1@example.com",
            nickname="u1",
            hashed_password=hash_password("Test123456"),
            role="user",
            is_active=True,
        ...
        monkeypatch.setattr(test_session, "commit", bad_commit, raising=True)
        ...
        monkeypatch.setattr(test_session, "commit", original_commit, raising=True)

        assert res.status_code == 200
        data = _json_dict(res)
        assert data.get("session_id") == "s_persist_fail_1"
        assert data.get("answer") == "OK"
        assert data.get("assistant_message_id") is None
        assert isinstance(res.headers.get("X-Request-Id"), str)
