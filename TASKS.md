...
- [ ] 后台内容上传能力：封面从“链接”升级为“上传”（并可扩展到其他模块）
  - [ ] News 封面：后台编辑支持上传图片生成 URL（仍兼容手动填写链接）
  - [ ] 将“上传-存储-返回访问 URL”的能力做成通用模块，其他需要图片/文件的模块同步接入
  - [ ] 约束：校验文件类型/大小；前端上传进度与失败提示；后端存储路径与清理策略明确
- [ ] 支付管理实用化：/admin/payment-callbacks 与渠道密钥维护
  - [ ] 支付管理页支持替换/更新新的渠道密钥（不暴露敏感信息，仅支持覆盖更新与状态校验）
  - [ ] 渠道配置状态更清晰：哪些字段缺失、哪些回调 URL 生效、最近验证时间与结果
  - [ ] 支付回调审计的筛选/检索体验优化（按渠道/时间/订单号/流水号）
- [ ] 初始化数据脚本完善
  - [ ] 补齐一键初始化：管理员账号、默认 SystemConfig（非敏感项）、必要的模板/知识库示例数据
  - [ ] 明确可重复执行策略（幂等/覆盖/跳过），并输出清晰日志
- [ ] 管理后台侧边栏信息架构整理
