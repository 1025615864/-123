...
import { Bell, Check, Trash2, MessageSquare, Heart, Bookmark, AlertCircle } from 'lucide-react'
...
import { useNotificationsPreviewQuery, type NotificationsPreviewResponse } from '../queries/notifications'
...
export default function NotificationBell({ tone = 'dark' }: { tone?: 'dark' | 'light' }) {
  ...
  const { queryKey: previewQueryKey, query: previewQuery } = useNotificationsPreviewQuery(10, isAuthenticated)
  ...
  const markAsReadMutation = useMutation({
    ...
    onMutate: async (id) => {
      ...
      const previous = queryClient.getQueryData<NotificationsPreviewResponse>(previewQueryKey)

      queryClient.setQueryData<NotificationsPreviewResponse>(previewQueryKey, (old) => {
    ...
    onError: (err, _id, ctx) => {
      if (ctx?.previous) queryClient.setQueryData(previewQueryKey, ctx.previous)
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <AlertCircle className={`h-4 w-4 ${isLight ? 'text-gray-500' : 'text-white/50'}`} />
    }
  }
  ...
  return (
    <div className="relative">
      ...
      >
        <Bell className={`h-5 w-5 ${isLight ? 'text-gray-700' : 'text-white/70'}`} />
        {unreadCount > 0 && (
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            >
              <h3 className={`font-semibold ${isLight ? 'text-gray-900' : 'text-white'}`}>通知消息</h3>
              {unreadCount > 0 && (
                <Button 
                  variant="ghost" 
                  size="sm" 
            ...
            <div className="max-h-80 overflow-y-auto">
              {notifications.length === 0 ? (
                <div className={`py-12 text-center ${isLight ? 'text-gray-500' : 'text-white/40'}`}>
                  <Bell className={`h-8 w-8 mx-auto mb-2 ${isLight ? 'text-gray-400' : 'opacity-50'}`} />
                  <p>暂无通知</p>
                </div>
              ) : (
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {notification.content && (
                          <p className={`text-xs mt-1 line-clamp-2 ${isLight ? 'text-gray-600' : 'text-white/50'}`}>
                            {notification.content}
                          </p>
                        )}
                        <div className="flex items-center justify-between mt-2">
                          <span className={`text-xs ${isLight ? 'text-gray-500' : 'text-white/40'}`}>
            ...
            {notifications.length > 0 && (
              <div className={`px-4 py-2 border-t text-center ${isLight ? 'border-gray-200' : 'border-white/10'}`}>
                <Link
                  to="/notifications"
                  className={`text-sm ${
                    isLight ? 'text-emerald-700 hover:text-emerald-800' : 'text-amber-400 hover:text-amber-300'
