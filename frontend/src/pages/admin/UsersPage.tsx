...
export default function UsersPage() {
  ...
  const [activeToggleUserId, setActiveToggleUserId] = useState<number | null>(
    null
  );
  const pageSize = 20;
  ...
  useEffect(() => {
    ...
    toast.error(
      getApiErrorMessage(usersQuery.error, "用户列表加载失败，请稍后重试")
    );
  ...
  const toggleActiveMutation = useAppMutation<void, User>({
    ...
    onMutate: async (u) => {
      setActiveToggleUserId(u.id);
    },
    onSettled: (_data, _err, u) => {
      setActiveToggleUserId((prev) => (prev === u?.id ? null : prev));
    },
  });

  const saveRoleMutation = useAppMutation<
    void,
    { userId: number; role: string }
  >({
    mutationFn: async (payload) => {
      await api.put(
        `/user/admin/${payload.userId}/role?role=${payload.role}`,
        {}
      );
    },
    errorMessageFallback: "操作失败，请稍后重试",
  ...
  const detailModalBusy =
    ...
    (!!selectedUser &&
      ...
      activeToggleUserId === selectedUser.id);
      ...
      >
        {selectedUser && (
          <div className="space-y-6">
            ...
            {userStatsQuery.isLoading && !userStats ? (
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button
                ...
                loadingText="保存中..."
                disabled={saveRoleMutation.isPending || !newRole}
                className="flex-1"
              >
                保存
              </Button>
