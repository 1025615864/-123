...
async function getPostReviewConfig(request: any, adminToken: string) {
  ...
  return (await res.json()) as {
    ...
    post_review_mode: string;
  };
}

async function setPostReviewConfig(
  request: any,
  adminToken: string,
  enabled: boolean,
  mode: string
...
async function setCommentReviewConfig(
  request: any,
  adminToken: string,
  enabled: boolean
) {
...
async function getAnyPostId(request: any, userToken?: string) {
  if (userToken) {
    const now = Date.now();
    ...
    const approveRes = await request.post(
      `${apiBase}/forum/admin/posts/${postId}/review`,
      {
        headers: { Authorization: `Bearer ${adminToken}` },
        data: { action: "approve", reason: "E2E auto-approve host post" },
      }
    );
...
async function createComment(
  ...
  if (!res.ok()) {
    ...
    throw new Error(`createComment failed: status=${res.status()} body=${body}`);
  }
  const json = await res.json();
...
async function rejectComment(
  ...
  const res = await request.post(
    `${apiBase}/forum/admin/comments/${commentId}/review`,
    {
      headers: { Authorization: `Bearer ${adminToken}` },
...
async function batchReviewComments(
  ...
  const res = await request.post(
    `${apiBase}/forum/admin/comments/review/batch`,
    {
      headers: { Authorization: `Bearer ${adminToken}` },
...
async function waitForHighlightToAppearAndDisappear(
  page: any,
  commentId: number
) {
...
test("通知跳转 commentId 深链：自动滚动+高亮，且驳回原因可见、交互按钮隐藏", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  const postId = await getAnyPostId(request, user.token);
...
test("我的评论跳转 commentId 深链：自动滚动+高亮", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  const myCommentLink = page
    .locator(`a[href*="commentId=${commentId}"]`)
    .first();
...
test("待审评论（pending）：作者可见“审核中”，且回复/点赞按钮不显示", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  await page.goto(
    `/forum/post/${postId}?commentId=${commentId}#comment-${commentId}`
  );
  ...
  await expect(el.getByRole("button", { name: "回复" })).toHaveCount(0);
  await expect(el.locator("svg.lucide-thumbs-up")).toHaveCount(0);
});

test("切换账号后不泄露待审帖子内容：非作者访问显示 403/404 空态且旧标题不可见", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: pendingPostTitle })
  ...
  await expect(
    page.getByText(/该内容可能正在审核中或已被驳回|帖子不存在/)
  ).toBeVisible({ timeout: 12_000 });
  await expect(
    page.getByRole("heading", { level: 1, name: pendingPostTitle })
  ).toHaveCount(0);
...
test("帖子被驳回（rejected）：作者可见驳回提示+原因；切换到非作者后 403/404 空态且标题不泄露", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("帖子被驳回通知：notifications -> post；返回后通知标记已读", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  await expect(row).toBeVisible({ timeout: 12_000 });
  await expect(row.locator('button[title="标记已读"]')).toBeVisible({
    timeout: 12_000,
  });
...
test("帖子待审通知：notifications -> post；展示“你的帖子正在审核中”+原因；返回后通知标记已读", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  try {
    ...
    await setPostReviewConfig(
      request,
      adminToken,
      !!prevConfig.post_review_enabled,
      prevConfig.post_review_mode
    );
  }
});

test("评论待审通知：notifications -> post(commentId) 深链；自动滚动+高亮；显示“审核中”且无交互按钮", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("评论被删除通知：notifications -> post(commentId) 深链；自动滚动+高亮；显示“已驳回”(title 含原因)且无交互按钮；返回后通知已读", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("关闭评论审核开关 enabled=false：评论应直接 approved 且不产生待审通知", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  try {
    ...
    await page.goto("/notifications");
    await expect(
      page.locator("div.p-5").filter({ hasText: "你的评论已提交审核" })
    ).toHaveCount(0);
  } finally {
    await setCommentReviewConfig(
      request,
      adminToken,
      !!prevConfig.comment_review_enabled
    );
  }
});

test("通知中心分页：制造>20条通知，翻页后可见不同链接，未读数正确", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  try {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      commentIds.push(
        await createComment(request, postId, user.token, content)
      );
    }
    ...
    const page1ResPromise = page.waitForResponse((res) => {
      const url = res.url();
      return (
        res.request().method() === "GET" &&
        url.includes("/api/notifications") &&
        url.includes("page=1") &&
        url.includes("page_size=20") &&
        res.ok()
      );
    });
    await page.goto("/notifications");
    const page1Res = await page1ResPromise;
    const page1Json = await page1Res.json();
    const firstIdPage1 = Number(page1Json?.items?.[0]?.id || 0);
    expect(firstIdPage1).toBeGreaterThan(0);

    const unreadLabel = page.getByText(/未读\s*\d+\s*条/).first();
    await expect(unreadLabel).toBeVisible({ timeout: 12_000 });
    const unreadText = (await unreadLabel.textContent()) || "";
    const unreadMatch = unreadText.match(/未读\s*(\d+)\s*条/);
    expect(unreadMatch).toBeTruthy();
    expect(Number(unreadMatch?.[1] || 0)).toBeGreaterThanOrEqual(25);

    const itemRows = page.locator("div.p-5.flex");
    await expect(itemRows).toHaveCount(20, { timeout: 12_000 });

    const firstHrefPage1 = await page
      .getByRole("link", { name: "查看链接" })
      .first()
      .getAttribute("href");
    expect(firstHrefPage1).toBeTruthy();

    const pager = page.locator("div.p-5.border-t").first();
    const page2ResPromise = page.waitForResponse((res) => {
      const url = res.url();
      return (
        res.request().method() === "GET" &&
        url.includes("/api/notifications") &&
    ...
    const page2Res = await page2ResPromise;
    const page2Json = await page2Res.json();
    const firstIdPage2 = Number(page2Json?.items?.[0]?.id || 0);
    expect(firstIdPage2).toBeGreaterThan(0);
    expect(firstIdPage2).not.toBe(firstIdPage1);

    await expect(itemRows.first()).toBeVisible({ timeout: 12_000 });
    const page2Count = await itemRows.count();
    expect(page2Count).toBeGreaterThan(0);
    expect(page2Count).toBeLessThanOrEqual(20);

    const firstHrefPage2 = await page
      .getByRole("link", { name: "查看链接" })
      .first()
      .getAttribute("href");
    ...
    await expect(itemRows).toHaveCount(20, { timeout: 12_000 });
    const firstHrefPage1Again = await page
      .getByRole("link", { name: "查看链接" })
      .first()
      .getAttribute("href");
...
test("草稿箱：自动保存草稿，草稿列表可见，继续编辑可还原内容，删除草稿后为空", async ({
  page,
}) => {
  const now = Date.now();
  ...
  await page
    .getByPlaceholder("请输入内容，支持 Markdown、表情、图片和附件链接...")
    .fill(content);
  ...
  await expect(
    page.getByPlaceholder("请用一句话描述你的问题/观点")
  ).toHaveValue(title, { timeout: 12_000 });
  await expect(
    page.getByPlaceholder("请输入内容，支持 Markdown、表情、图片和附件链接...")
  ).toHaveValue(content, {
    timeout: 12_000,
  });
...
test("回收站：批量恢复与批量永久删除", async ({ page, request }) => {
  ...
  await expect(page.getByText(p1Title).first()).toBeVisible({
    timeout: 12_000,
  });
  await expect(page.getByText(p2Title).first()).toBeVisible({
    timeout: 12_000,
  });
  ...
  const q1 = await createPost(
    request,
    user.token,
    q1Title,
    `E2E 回收站永久删内容 ${now}-1`
  );
  const q2 = await createPost(
    request,
    user.token,
    q2Title,
    `E2E 回收站永久删内容 ${now}-2`
  ...
  await page.goto("/forum/recycle-bin");
  await expect(page.getByText(q1Title).first()).toBeVisible({
    timeout: 12_000,
  });
  ...
  await expect(page.getByText("批量永久删除结果")).toBeVisible({
    timeout: 12_000,
  });
  await page.getByRole("button", { name: "知道了" }).click();
  await expect(page.getByText("回收站为空")).toBeVisible({ timeout: 12_000 });
...
test("批量通知合并（评论>10 delete）：仅 1 条（批量）删除通知 + 深链可用 + 作者侧已驳回/禁交互", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  try {
    ...
    const row = page
      ...
      .first();
    await expect(row).toBeVisible({ timeout: 12_000 });
    await expect(row.getByText("评论ID").first()).toBeVisible({
      timeout: 12_000,
    });
    await expect(row.locator('button[title="标记已读"]')).toBeVisible({
      timeout: 12_000,
    ...
    expect(
      Number.isFinite(targetCommentId) && targetCommentId > 0
    ).toBeTruthy();
...
test("批量通知合并（帖子>10 approve）：仅 1 条（批量）通过通知 + 跳转可用 + 点击后已读", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  const row = page
    ...
    .first();
  await expect(row).toBeVisible({ timeout: 12_000 });
  await expect(row.getByText("帖子ID").first()).toBeVisible({
    timeout: 12_000,
  });
  await expect(row.locator('button[title="标记已读"]')).toBeVisible({
    timeout: 12_000,
  ...
  await expect(
    page
      ...
      .filter({ hasText: `E2E 合并批量通过帖子 ${now}-` })
  ).toBeVisible({ timeout: 12_000 });
...
test("批量通知合并（帖子>10 reject）：仅 1 条（批量）驳回通知 + 作者可见原因 + 点击后已读", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  await expect(
    page
      .getByRole("heading", { level: 1 })
      .filter({ hasText: `E2E 合并批量驳回帖子 ${now}-` })
  ).toBeVisible({ timeout: 12_000 });
  await expect(page.getByText("你的帖子未通过审核").first()).toBeVisible({
    timeout: 12_000,
  });
...
test("通知中心：刷新按钮可拉取新通知（页面打开后创建）", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  try {
    ...
    await page.goto("/notifications");
    await page
      .getByRole("heading", { level: 1, name: "通知中心" })
    ...
    await expect(
      page.locator("div.p-5").filter({ hasText: reason }).first()
    ).toBeVisible({ timeout: 12_000 });
  } finally {
    await setCommentReviewConfig(
      request,
...
test("ForumPage 列表可见性：我的帖子显示审核状态；全部不显示未审核帖子", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  try {
    ...
    // 全部列表不应出现未审核帖子
    await expect(page.locator("a").filter({ hasText: postTitle })).toHaveCount(
      0
    );
    ...
    await expect(myCard.getByText("审核中").first()).toBeVisible({
      timeout: 12_000,
    });
    ...
    await setPostReviewConfig(
      request,
      adminToken,
      !!prevPostCfg.post_review_enabled,
      prevPostCfg.post_review_mode
    );
  }
});
...
test("通知中心：全部已读（清空所有未读标记）", async ({ page, request }) => {
  ...
  try {
    ...
    await expect(page.getByText(/未读\s+\d+\s+条/)).toBeVisible({
      timeout: 12_000,
    });
...
test("通知中心：删除通知（从列表移除并更新未读计数）", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  const prevUnreadText = await page
    .getByText(/未读\s+\d+\s+条/)
    .first()
    .textContent();
...
test("帖子批量 reject 后非作者访问不泄露标题（同会话切换账号）", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  const postId1 = await createPost(
    request,
    author.token,
    postTitle,
    postContent
...
test("批量通知合并（评论>10 reject）：仅 1 条（批量）通知 + 深链可用 + 作者侧已驳回/禁交互", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("批量通知合并（帖子>10 delete）：仅 1 条（批量）通知 + 跳转 deleted=1 回收站视图", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("批量通知合并（评论>10 approve）：仅 1 条（批量）通知 + 深链可用 + 作者侧审核中->可交互", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("帖子批量 delete 后非作者访问不泄露标题（同会话切换账号）", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  await page.goto(href);
  await expect(
    page.getByText(/仅作者或管理员可查看|暂无权限|帖子不存在/)
  ).toBeVisible({ timeout: 12_000 });
  await expect(
    page.getByRole("heading", { level: 1, name: firstTitle })
  ).toHaveCount(0);
...
test("管理员批量审核评论（reject）：作者侧可见“已驳回”(title 含原因)且无交互，并收到（批量）驳回通知", async ({
  page,
  request,
}) => {
  const now = Date.now();
  ...
  try {
    ...
    await page.goBack();
    await page.waitForURL("**/notifications", { timeout: 12_000 });
    const rowAfter = getNotificationRow(
      page,
      href,
      "你的评论未通过审核（批量）"
    );
...
test("管理员批量审核评论（delete）：作者侧可见“已驳回”(title 含原因)且无交互，并收到（批量）删除通知", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("帖子批量审核（approve）：作者可正常查看帖子，并收到（批量）通过通知（点击后已读）", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("帖子批量审核（reject）：作者可见驳回提示+原因，并收到（批量）驳回通知（点击后已读）", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("帖子批量审核（delete）：作者在回收站视图可见“该帖子已删除”，并收到（批量）删除通知（点击后已读）", async ({
  page,
  request,
}) => {
  const now = Date.now();
...
test("管理员批量审核评论（approve）：作者侧从“审核中”变为可交互，并收到（批量）通过通知", async ({
  page,
  request,
}) => {
  const now = Date.now();
      ...
      .poll(
        async () => {
          const res = await request.get(
            `${apiBase}/forum/posts/${postId}/comments?include_unapproved=1`,
            {
