...
test("前台：Chat 流式期间上滑锁定 + 回到底部按钮（E2E mock）", async ({
  page,
}) => {
  ...
  // Simulate user scrolling up (lock auto-scroll) using real wheel events,
  // so React onScroll/onWheel handlers are reliably triggered.
  ...
  const scrollBtn = page.locator('button[aria-label="回到底部"]');
